      ...
      <CardContent>
        <div
          className={`border-2 border-dashed rounded-lg p-6 md:p-8 text-center transition-colors ${
            dragActive ? "border-primary bg-primary/10 dark:bg-primary/20" : "border-border hover:border-primary/50"
          }`}
          ...
          onDrop={handleDrop}
        >
          <FileText className="w-10 h-10 md:w-12 md:h-12 mx-auto mb-4 text-muted-foreground" />
          <div className="space-y-2">
            <p className="text-base md:text-lg font-medium">
              {isUploading ? "AI分析中..." : "拖拽文件到这里或点击上传"}
            </p>
            <p className="text-xs md:text-sm text-muted-foreground">支持 .txt, .md, .doc, .docx, .pdf 格式，最大 25MB</p>
          </div>
          ...
          {isUploading ? (
            <div className="mt-4 w-full">
              ...
              <p className="text-sm text-muted-foreground mt-2">{Math.round(uploadProgress)}% 完成</p>
            </div>
          ) : (
