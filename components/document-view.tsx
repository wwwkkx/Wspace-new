...
import { FileText, Tag, Calendar, Edit, Save, ArrowLeft, Trash2, Download } from "lucide-react"
  ...
  const fetchDocument = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setDocument(data)
        // 文档内容现在从API中获取，已经包含在originalContent字段中
        ...
        <div className="border rounded-lg">
          <div className="bg-gray-50 p-3 border-b">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <h3 className="font-medium">原始文档</h3>
              </div>
              <Button 
                variant="outline" 
                size="sm" 
          ...
          <div className="p-6 flex flex-col items-center justify-center">
            ...
            <div className="text-center">
              <p className="font-medium mb-1">{document.fileName}</p>
              <p className="text-sm text-gray-500">{document.fileSize ? `${(document.fileSize / 1024).toFixed(2)} KB` : ''}</p>
            </div>
          </div>
        </div>
      </CardContent>
