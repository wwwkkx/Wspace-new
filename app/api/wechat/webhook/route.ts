...
async function processUserMessage(userId: string, content: string, userAuth: any) {
  ...
  // 准备用户授权信息
  const notionAuthData = {
    notionToken: userAuth.notionToken,
    databaseId: userAuth.databaseId,
    workspaceName: userAuth.workspaceName,
    workspaceId: userAuth.workspaceId,
    botId: userAuth.botId,
    databaseName: userAuth.databaseName,
  }
  ...
  const notionResponse = await fetch(`${process.env.NEXTAUTH_URL}/api/notion/save-diary`, {
    ...
    body: JSON.stringify({
      userId,
      analysis,
      userAuth: notionAuthData,
    }),
  })

  if (!notionResponse.ok) {
    const errorData = await notionResponse.json()
    console.error("Notion save failed:", errorData)
    throw new Error(`Notion save failed: ${errorData.error || 'Unknown error'}`)
  }
